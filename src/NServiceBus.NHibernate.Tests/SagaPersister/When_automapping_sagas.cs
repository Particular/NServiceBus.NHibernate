...
public class When_autoMapping_sagas
{
    ...
    [Test]
    public void Id_generator_should_be_set_to_assigned() => Assert.That(persisterForTestSaga.IdentifierGenerator.GetType(), Is.EqualTo(typeof(Assigned)));

    [Test]
    public void Enums_should_be_mapped_as_integers() => persisterForTestSaga.ShouldContainMappingsFor<Status>();
    ...
    [Test]
    public void Datetime_properties_should_be_mapped() => persisterForTestSaga.ShouldContainMappingsFor<DateTime>();

    [Test]
    public void Public_setters_and_getters_of_concrete_classes_should_map_as_components() => persisterForTestSaga.ShouldContainMappingsFor<TestComponent>();
...
{
    public static IEntityPersister GetEntityPersisterFor<T>(this SessionFactoryImpl sessionFactory) => sessionFactory.GetEntityPersister(typeof(T).FullName);

    public static void ShouldContainPersisterFor<T>(this SessionFactoryImpl sessionFactory) => Assert.That(sessionFactory.GetEntityPersisterFor<T>(), Is.Not.Null);
}
