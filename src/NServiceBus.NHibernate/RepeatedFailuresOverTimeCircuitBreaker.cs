namespace NServiceBus
{
    ...
    sealed class RepeatedFailuresOverTimeCircuitBreaker
    {
            ...
            TimeSpan timeToWaitWhenArmed = default)
        {
            ...
            this.timeToWaitWhenTriggered = timeToWaitWhenTriggered == TimeSpan.MinValue
                ? TimeSpan.FromSeconds(10)
                : timeToWaitWhenTriggered;
            this.timeToWaitWhenArmed =
                timeToWaitWhenArmed == TimeSpan.MinValue ? TimeSpan.FromSeconds(1) : timeToWaitWhenArmed;
        ...
        public Task Failure(Exception exception, CancellationToken cancellationToken = default)
        {
            ...
            Task Delay()
            {
                ...
                {
                    Logger.DebugFormat("The circuit breaker for '{0}' is delaying the operation by '{1}'.", name,
                        timeToWait);
                }
        ...
        void CircuitBreakerTriggered(object state)
        {
            ...
            lock (stateLock)
            {
                ...
                Logger.WarnFormat("The circuit breaker for '{0}' will now be triggered with exception '{1}'.", name,
                    lastException);
